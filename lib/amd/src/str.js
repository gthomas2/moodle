...
define(['jquery', 'core/ajax', 'core/localstorage'], function($, ajax, storage) {
    ...
    /** @var {string{}} strRequests - String requests in progress */
    var strRequests = {};
    ...
    return /** @alias module:core/str */ {
        ...
        /**
         * Make request key from request object.
         *
         * @param {Object} request
         */
        // eslint-disable-next-line camelcase
        request_key: function(request) {
            return 'core_str/' + request.key + '/' + request.component + '/' + request.lang;
        },
        ...
        get_strings: function(requests) {
            ...
            for (i = 0; i < requests.length; i++) {
                ...
                if (typeof M.str[request.component] === "undefined" ||
                    ...
                    var cached = storage.get(self.request_key(request));
                    if (cached) {
                        if (typeof M.str[request.component] === "undefined") {
            ...
            if (!missing) {
                ...
                for (i = 0; i < requests.length; i++) {
                    request = requests[i];
                    reqKeys.push(self.request_key(request));
                ...
                // Create unique request key by joining all requested strings together - we are not trying to avoid
                // duplicate individual string requests between multiple string requests - we are trying to avoid
                // duplicate requests for the same group of strings (e.g. get_strings called in iterator).
                var reqKey = reqKeys.join('~');
                if (reqKey in strRequests) {
                    // Return promise for existing request
                    return strRequests[reqKey];
                } else {
                    // Add promise for reqKey.
                    strRequests[reqKey] = deferred.promise();
                }
                ...
                $.when.apply(null, deferreds).done(
                    function() {
                        ...
                        for (i = 0; i < arguments.length; i++) {
                            ...
                            storage.set(self.request_key(request), arguments[i]);
                            // And set the results.
                    ...
                    function() {
                        // Remove reqKey from strRequests.
                        delete strRequests[reqKey];
                    }
                );
            }
